...
module.exports = {
    ...
    execute(message, args) {
        ...
        if (args[2] === '/') var num = (args[1] * 1) / (args[3] * 1);
        if (args[2] === '^') var num = (args[1] * 1) ** (args[3] * 1);
        if (args[1] === '0' && args[2] === '/' && args[3] === '0') var num = `Imagine that you have zero cookies and you split them evenly among zero friends. How many cookies does each person get? See? It doesn't make sense. And Cookie Monster is sad that there are no cookies. And you are sad that you have no friends.`;
